...
def main():
    ...
    try:
        ...
        if args.config_file:
            ...
            # 从配置中获取各类媒体数据
            video_data_list = config.get('video_json', [])
            audio_data_list = config.get('audio_json', [])
            voice_data_list = config.get('voice_json', [])
            
            print(f"从配置文件读取: 视频数据 {len(video_data_list)}项, 音频数据 {len(audio_data_list)}项, 配音数据 {len(voice_data_list)}项")
        else:
            # 使用命令行参数
            if not args.material_dir or not args.draft_path:
                print("错误: 当不使用总配置文件时，必须提供--material_dir和--draft_path参数")
                return 1
